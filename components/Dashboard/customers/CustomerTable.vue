  ...
  >
    <div class="rounded-t mb-0 px-4 py-3 border-0">
      ...
      <div class="flex justify-start items-start mt-6 mb-6">
        ...
        <div class="shrink-0 w-40 px-5">
          <button @click="totalTrash > 0 ? $router.push({path: `/dashboard/customers/trash?type=${context.trash_type}`}) : ''" type="button" class="inline-block px-4 py-1.5 bg-red-600 text-white font-medium text-xs leading-tight uppercase rounded shadow-md hover:bg-red-700 hover:shadow-lg focus:bg-red-700 focus:shadow-lg focus:outline-none focus:ring-0 active:bg-blue-800 active:shadow-lg transition duration-150 ease-in-out">
        ...
        <div>
          <dashboard-molecules-globals-modal
            :showModal="showModal"
            :context="context"
            endPoint="/fitur/customer-management"
            :paging="paging"
            @toggle-modal="toggleModal"
            @show-toast-message="showToastMessage"
            @update-data="fetchAllCustomerData"
          />
        </div>
      </div>
      <div
           ...
           <td
            ...
            >
              <dashboard-molecules-globals-table-dropdown
                ...
                " :data="product.id" :context="context" @show-toast-message="showToastMessage" endPoint="/fitur/customer-management" :token="token"
              />
            </td>
          </tr>
    ...
    <div class="mt-12 mb-12">
      <div class="flex justify-end items-end">
        <dashboard-molecules-globals-pagination :links="links" :paging="paging" @fetch-product="fetchAllCustomerData" />
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  mounted() {
    this.checkIsLogin();
    this.fetchAllCustomerData(true, false, this.paging.current, '', '');
    this.fetchAllCustomerTrash(true);
  },
  ...
  methods: {
    ...
    fetchAllCustomerData(loading, loadingDelete, page=1, name='', phone='', email=''){
      try {
        this.empty = false
        ...
        const endPoint = `${this.api_url}/fitur/customer-management?page=${page}&name=${name}&phone=${phone}&email=${email}`
      ...
      try {
        this.$swal({
          ...
          if (result.isConfirmed) {
            ...
            const endPoint = `${this.api_url}/fitur/customer-management/${product_id}`;
    ...
    fetchAllCustomerTrash(loading) {
      try {
        ...
        const endPoint = `${this.api_url}/fitur/total-trash?type=CUSTOMER_DATA`;
        this.$axios
          .get(endPoint, config)
          .then(({ data }) => {
            if (data?.data > 0) {
  ...
  watch: {
    notifs() {
      if (this.notifs.length > 0) {
        if(this.context.title === 'Customer') {
          // console.log(this.notifs);
          this.fetchAllCustomerData(false, true, this.paging.current, '', '', '');
          this.fetchAllCustomerTrash(true)
        }
      }
    }
