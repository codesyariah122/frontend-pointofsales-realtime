...
export default {
  ...
  methods: {
    ...
    fetchAllCustomerTrash(loading, loadingDelete) {
      try {
      ...
      try {
        this.$swal({
          ...
          if (result.isConfirmed) {
            ...
            const endPoint = `${this.api_url}/fitur/trashed/${customer_id}/?type=${this.trash_type}`;
            this.$axios
              .put(endPoint)
              .then(({ data }) => {
                if (data?.success) {
    ...
    deleteProduct(id) {
      try {
        this.$swal({
          title: "Are you sure?",
          text: "Yakin ingin menghapus customer !",
          icon: "warning",
          showCancelButton: true,
          confirmButtonColor: "#3085d6",
          cancelButtonColor: "#d33",
          ...
          if (result.isConfirmed) {
            ...
            const endPoint = `${this.api_url}/fitur/trashed/${customer_id}/?type=CUSTOMER_DATA`;
            this.$axios
  ...
  watch: {
    notifs() {
      if (this.notifs.length > 0) {
        this.fetchAllCustomerTrash(false, true, 0, '', '');
      }
    },
