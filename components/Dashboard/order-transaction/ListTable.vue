...
export default {
  ...
  mounted() {
    this.checkIsLogin();
    this.fetchAllOrderProduct(true, false, this.paging.current, '', '');
    this.fetchAllOrderTrash(true);
  },
  ...
  methods: {
    ...
    fetchAllOrderProduct(loading, loadingDelete, page=1, name='', barcode='', category_id=this.queryCategory){
      try {
        this.empty = false
        ...
        const endPoint = `${this.api_url}/fitur/order-management?page=${page}&name=${name}&barcode=${barcode}&category_id=${category_id}`
        ...
        .then(({data}) => {
          if(data.data.data.length > 0) {
            ...
            this.products.map(item => {
              this.getQrCode(item.barcode, item.invoice_number)
            })
          } else {
            this.products = []
            this.links = []
    ...
    searchByName(e) {
      this.fetchAllOrderProduct(false, false, 1, '', '', '')
  ...
  watch: {
    notifs() {
      if (this.notifs.length > 0) {
        if(this.context.title === 'Product') {
          ...
          this.fetchAllOrderProduct(false, true, this.paging.current, '', '', '');
          this.fetchAllOrderTrash(true)
        }
      }
    },
    queryName() {
      if(this.queryName !== '') {
        this.fetchAllOrderProduct(true, false, 0, this.queryName, '')
      }
    },
    queryBarcode() {
      if(this.queryBarcode !== '') {
        this.fetchAllOrderProduct(false, false, 0, '', this.queryBarcode, '')
      }
    },
    queryQrcode() {
      if(this.queryQrcode !== '') {
        this.fetchAllOrderProduct(false, false, 0, '', this.queryQrcode, '')
      }
    },
    queryCategory() {
      if(this.queryCategory !== '') {
        this.fetchAllOrderProduct(true, false, 0, '', '', parseInt(this.queryCategory))
      }
    }
  },
};
