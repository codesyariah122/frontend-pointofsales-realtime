    ...
    <div class="relative w-auto my-6 mx-auto max-w-3xl">
      ...
      >
        <form @submit.prevent="updateData">
          ...
          >
            <h3 class="text-1xl font-semibold">Edit Data {{context.title}}</h3>
            <button
...
<script>
export default {
  props: ["data", "showModal", "context", "endPoint", "paging", "roles", "status", "loadingRole", "edit"],
  data() {
    return {
      loading: null,
      loadingAdd: null,
      ...
      validation: [],
      form: localStorage.getItem('prepare-edit') ? JSON.parse(localStorage.getItem('prepare-edit')) : {}
    };
  },
  ...
  methods: {
    ...
    updateData() {
      try {
        ...
        const endPoint = `${this.api_url}${this.endPoint}/${this.detail.id}`
        this.$axios.post(endPoint, this.form, config)
        .then(({data}) => {
          if(data.success) {
            this.toggleModal()
            this.$emit('show-toast-message', `Success update ${this.context.title} ${data.data[0].name}`, data.message)
          }
        })
        .finally(() => {
          setTimeout(() => {
